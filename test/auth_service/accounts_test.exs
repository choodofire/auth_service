defmodule AuthService.AccountsTest do
  use AuthService.Support.DataCase
  ...
  describe "create_account/1" do
    test "success: it insert an account in the db and returns the account" do
    ...
    test "errors: returns an error tuple when account can't be created" do
      missing_params = %{}

      assert {:error, %Ecto.Changeset{valid?: false}} = Accounts.create_account(missing_params)
    end
  end
  ...
  describe "update_account/2" do
    ...
    test "error: returns an error tuple when account can not be updated" do
      existing_account = Factory.insert(:account)
      bad_params = %{"email" => NaiveDateTime.utc_now()}

      assert {:error, %Ecto.Changeset{}} = Accounts.update_account(existing_account, bad_params)
